...
class ConnectionSolver(IConnectionSolver):
    ...
    def find_connections(self, query: ConnectionQuery) -> ConnectionResults:
        ...
        if start_stop_id is None:
            LOG.debug(f"ConnectionSolver({id(self)}): Start stop not found({query.start_stop_name})")
        ...
        if end_stop_id is None:
            LOG.debug(f"ConnectionSolver({id(self)}): End stop not found({query.end_stop_name})")
